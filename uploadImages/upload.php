...
//						$fileName = $fileName['filename'];
						...
						if($rs['errno'] == 0)
						{
							var_dump(json_encode($rs['data']['img']));exit;
							$arr = array();
							array_push($arr['s'],$rs['data']['img']['s']['url']);
							array_push($arr['t'],$rs['data']['img']['t']['url']);
							array_push($arr['n'],$rs['data']['img']['n']['url']);
							setDB(json_encode($arr),$file);
						}
					}
				}
			}
